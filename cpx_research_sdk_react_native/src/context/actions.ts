...
import { ISetNotificationWidgetHiding, ISetCpxState, IUpdateAppContext } from "./actionInterfaces";
...
export const setCpxState = (state: IAppStore, payload: ISetCpxState["payload"]): void => {
  {
    state.config.showLogs && console.log("[setCPXState] set state to " + payload.state);
  }
...
export const getWidgetImages = (state: IAppStore): void => {
  {
    state.config.showLogs && console.log("[getWidgetImages]");
  }

  const {
    config: { cornerWidget, notificationWidget, sidebarWidget },
  } = state;
